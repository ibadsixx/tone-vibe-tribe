...
AS $$
  SELECT CASE 
    ...
    WHEN post_audience_type = 'friends_except' THEN 
      (auth.uid() = post_user_id OR public.is_friend(auth.uid(), post_user_id))
      AND NOT (post_audience_excluded_user_ids IS NOT NULL AND auth.uid() = ANY(post_audience_excluded_user_ids))
    WHEN post_audience_type = 'specific' THEN 
      auth.uid() = post_user_id OR (post_audience_user_ids IS NOT NULL AND auth.uid() = ANY(post_audience_user_ids))
    WHEN post_audience_type = 'custom_list' THEN 
      auth.uid() = post_user_id OR 
      (post_audience_list_id IS NOT NULL AND auth.uid() = ANY((SELECT member_ids FROM audience_lists WHERE id = post_audience_list_id)))
    ELSE false
  END;
$$;
